...
**Institution:** Palacky University, Department of Geoinformatics, Olomouc
**Author:** Martin KRUPIČKA

The scripts cover:
- ASD spectrometer post-processing,
- ASD relative → approximate absolute reflectance correction,
...
- vignetting profile extraction and evaluation,
...
The uploaded repository currently contains these scripts:
...
- Packages used across the scripts:
  ...
  - `pillow-heif` *(optional; only if processing HEIC/HEIF images in `vignetting_analysis.py`)*

Typical installation:

```bash
pip install numpy pandas rasterio pillow tifffile
```
...
- `thermal_converter.py` requires **DJI Thermal SDK v1.8** and access to:
  - `dji_irp.exe`
- The script contains a default Windows path:
  - `C:\dji_termalsdk\sample\bin\windows\release_x64\dji_irp.exe`
...
```r
install.packages(c("terra", "ggplot2", "dplyr", "stringr", "readr", "tools", "tidyr", "zoo"))
```
...
- Most scripts use **absolute Windows paths** hard-coded in the source.
...
- The scripts were developed for a specific thesis workflow.
  - They are reusable, but they are not general-purpose packaged software.
- Some workflows rely on **manually prepared folders** and **consistent naming of plots / bands / heights**.
- Some of the R scripts generate plots with titles, subtitles, axis labels, and annotations written in Czech, as they were originally prepared for thesis outputs. When reusing these scripts, the text elements should be translated into the desired language and, if needed, adjusted to the specific devices, classes, surfaces, dates, or figure conventions of the new workflow.
- Before the first run, it is recommended to open the selected script and check:
  - input paths,
  - output paths,
  - filename rules,
  - expected folder structure,
  - any constants specific to your dataset.
...
- adapting the scripts for similar DJI / ASD / FLIR datasets,
...
- reads columns `Wavelength`, `Reflectance`, and `Source`,
- computes:
  - `Reflectance_abs95 = Reflectance * PANEL_REFLECTANCE`
- optionally clips the corrected reflectance values to the interval `[0, 1]`,
...
- applies the calibration to a folder of multispectral GeoTIFFs,
...
- This script is intended for single-band multispectral GeoTIFF inputs.
...
- detects the RAW dimensions / data type from file size,
- converts values to °C,
- saves one float32 TIFF per input thermal image,
...
- The default GUI path points to a Windows SDK installation.
...
**Outputs:**
- `gray/` *(grayscale TIFF copies for non-TIFF inputs)*
- `summary.csv`
- `profiles_H.csv`
- `profiles_V.csv`
- `profiles_D1.csv`
...
**Notes:**
- HEIC / HEIF support requires uncommenting the `pillow_heif` import lines in the script.
- This script runs as a normal Python script; it is **not** an ArcGIS tool.
...
- computes plot-level metrics per height and overall,
...
- The ASD reference label used in plots is `0.25m (ASD)`.
...
- computes:
  ...
  - percent MAE,
  - percent bias,
  - overall MAE / RMSE / AvgBias across heights,
...
`raster_to_tiff.py` is intended to be added and run as an **ArcGIS Pro Script Tool**.
...
4. Calibrate multispectral rasters to reflectance using `radiometric_converter.py`.
5. Clip calibrated rasters to ROI polygons using `raster_to_tiff.py` in ArcGIS Pro.
6. Compute statistics and generate boxplots using `multispec_r.R`.
...
- Verify that hard-coded Windows paths were updated.
...
### `raster_to_tiff.py` fails
- Run it inside ArcGIS Pro with a valid ArcPy environment.
...
### `vignetting_analysis.py` does not open HEIC files
- Install `pillow-heif`.
- Uncomment the `pillow_heif` import lines in the script.
