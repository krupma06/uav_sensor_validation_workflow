...
# --------------------------
dirs <- list(
    "40m"         = "D:/DP/250908_chomoutovska_bc_data/250908_chomoutovska_bc_data/termo/Outputs/termo_40",
    "80m"         = "D:/DP/250908_chomoutovska_bc_data/250908_chomoutovska_bc_data/termo/Outputs/termo_80",
    "120m"        = "D:/DP/250908_chomoutovska_bc_data/250908_chomoutovska_bc_data/termo/Outputs/termo_120",
    "FLIR_median" = "D:/DP/250908_chomoutovska_bc_data/250908_chomoutovska_bc_data/termo/FLIR_median"
)

# --------------------------
# 2) Labels
# --------------------------
FLIR_LABEL <- "1.5m (FLIR)"
KONTAKTNI_LABEL <- "kontaktní teploměr"
...
# 5) Output folder
# --------------------------
output_dir <- "D:/DP/250908_chomoutovska_bc_data/250908_chomoutovska_bc_data/termo/Outputs/Graph"
...
metrics_per_height <- plot_stats %>%
    ...
    mutate(
        ...
        MAE_pct_median  = ifelse(abs(FLIR_median) > 1e-9, abs_err_median / FLIR_median * 100, NA_real_),
        Bias_pct_median = ifelse(abs(FLIR_median) > 1e-9, bias_median    / FLIR_median * 100, NA_real_)
    )
...
stats_one_csv <- bind_rows(
    ...
    metrics_overall %>%
        transmute(
            ...
            MAE_pct_median = NA_real_,
            Bias_pct_median = NA_real_,
            k,
            MAE_median,
            RMSE_median,
            AvgBias_median
